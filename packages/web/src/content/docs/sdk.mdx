---
title: SDK
description: Type-safe JS client for oracle-code server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The oracle-code JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control oracle-code programmatically.
...
Create an instance of oracle-code:

```javascript
import { createOracleCode } from "@oracle-code/sdk"

const { client } = await createOracleCode()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `oracle-code.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${oracleCode.server.url}`)

oracleCode.server.close()
```

## Client only

If you already have a running instance of oracle-code, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@oracle-code/sdk"
```
