...
You can add external tools to Oracle Code using the _Model Context Protocol_, or MCP.

Oracle Code supports both:
...
You can define MCP servers in your Oracle Code config under `mcp`. Add each MCP
with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="oracle-code.jsonc" {6}
{
...
```jsonc title="oracle-code.jsonc" {15}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Oracle Code automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Oracle Code will:
...
If the server requires authentication, Oracle Code will prompt you to authenticate when you first try to use it.
...
oracle-code mcp auth my-oauth-server

# List all MCP servers and their auth status
oracle-code mcp list

# Remove stored credentials
oracle-code mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Oracle Code will store the tokens securely in `~/.local/share/oracle-code/mcp-auth.json`.

---

## Manage

Your MCPs are available as tools in Oracle Code, alongside built-in tools. So you
can manage them through the Oracle Code config like any other tool.
...
```json title="oracle-code.json" {14}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="oracle-code.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="oracle-code.json" {4-7}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="oracle-code.json" {7-9}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "context7": {
