...
declare global {
  const OCODE_VERSION: string
  const OCODE_CHANNEL: string
}
...
export namespace Installation {
  ...
  export async function method() {
    ...
    for (const check of checks) {
      ...
      if (output.includes(check.name === "brew" ? "opencode" : "oracle-code-ai")) {
        return check.name
      }
    }
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        ...
        break
      case "npm":
        cmd = $`npm install -g oracle-code-ai@${target}`
        break
      case "pnpm":
        cmd = $`pnpm install -g oracle-code-ai@${target}`
        break
      case "bun":
        cmd = $`bun install -g oracle-code-ai@${target}`
        break
      case "brew": {
  ...
  export const VERSION = typeof OCODE_VERSION === "string" ? OCODE_VERSION : "local"
  export const CHANNEL = typeof OCODE_CHANNEL === "string" ? OCODE_CHANNEL : "local"
  ...
  export async function latest(installMethod?: Method) {
    ...
    return fetch(`${registry}/oracle-code-ai/${channel}`)
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })
