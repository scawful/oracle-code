---
title: Providers
description: Using any LLM provider in Oracle Code.
---

import config from "../../../config.mjs"
export const console = config.console

Oracle Code uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Oracle Code config.
...
in `~/.local/share/oracle-code/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Oracle Code
config.
...
Oracle Code Zen is a list of models provided by the Oracle Code team that have been
tested and verified to work well with Oracle Code. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Oracle Code Zen.
:::

1. Run the `/connect` command in the TUI, select oracle-code, and head to [opencode.ai/auth](https://opencode.ai/auth).
...
It works like any other provider in Oracle Code. And is completely optional to use
it.
...
To use Amazon Bedrock with Oracle Code:
...
1. You'll need either to set one of the following environment variables:
   ...
   Once you have one of the above, set it while running oracle-code.

   ```bash
   AWS_ACCESS_KEY_ID=XXX oracle-code
   ```
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for oracle-code to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX oracle-code
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX oracle-code
   ```
...
To use your GitHub Copilot subscription with oracle-code:
...
To use Google Vertex AI with Oracle Code:
...
2. Set the required environment variables:
   ...
   Set them while running oracle-code.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id oracle-code
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through oracle-code, you can always configure it yourself.
...
```jsonc title="~/.config/oracle-code/oracle-code.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Oracle Code conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Oracle Code conversation listed as a separate session.
...
You can configure oracle-code to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="oracle-code.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure oracle-code to use local models through LM Studio.

```json title="oracle-code.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure oracle-code to use local models through Ollama.

```json title="oracle-code.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Oracle Code:
...
3. Copy the API key for use in Oracle Code.
...
6. **Important**: Before using cloud models in Oracle Code, you must pull the model information locally:
...
Oracle Code Zen is a list of tested and verified models provided by the Oracle Code team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Oracle Code Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Oracle Code Zen**.

   ```txt
   /connect
   ```

3. Enter your Oracle Code API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your oracle-code config.
...
5. You can also customize them through your oracle-code config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' oracle-code
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group oracle-code
   ```
...
:::tip
You can use any OpenAI-compatible provider with oracle-code. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in oracle-code.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `oracle-code.json` file in your project directory:

   ```json title="oracle-code.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="oracle-code.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Oracle Code to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `oracle-code auth list` to see if the credentials
...
2. For custom providers, check the oracle-code config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your oracle-code config.
