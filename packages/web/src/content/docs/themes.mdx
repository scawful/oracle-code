...
With Oracle Code you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Oracle Code uses our own `oracle-code` theme.
...
Oracle Code comes with several built-in themes.
...
- Want Oracle Code to match their terminal's appearance
...
Oracle Code supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/oracle-code/themes/*.json` or `$XDG_CONFIG_HOME/oracle-code/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.oracle-code/themes/*.json`
4. **Current working directory** - Defined in `./.oracle-code/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/oracle-code/themes
vim ~/.config/oracle-code/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .oracle-code/themes
vim .oracle-code/themes/my-theme.json
```
