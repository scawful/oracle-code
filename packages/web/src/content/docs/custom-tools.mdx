---
title: Custom Tools
description: Create tools the LLM can call in oracle-code.
---
...
- Locally by placing them in the `.oracle-code/tool/` directory of your project.
- Or globally, by placing them in `~/.config/oracle-code/tool/`.
...
```ts title=".oracle-code/tool/database.ts" {1}
import { tool } from "@oracle-code/plugin"
...
```ts title=".oracle-code/tool/math.ts"
import { tool } from "@oracle-code/plugin"
...
```ts title=".oracle-code/tool/project.ts" {8}
import { tool } from "@oracle-code/plugin"
...
```python title=".oracle-code/tool/add.py"
import sys
...
```ts title=".oracle-code/tool/python-add.ts" {10}
import { tool } from "@oracle-code/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .oracle-code/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
