...
/**
 * HivemindView - Hivemind dashboard as a pane view
 * 
 * Adapted from DialogHivemind for embedding in the pane system.
 */
export interface HivemindViewProps {
  paneId: string
  isActive?: boolean
  initialTab?: TabName
}

export function HivemindView(props: HivemindViewProps) {
  ...
  const ownerId = `hivemind-view:${props.paneId}`
  ...
  // Load hivemind state using afs.root from context
  // The source includes both refreshTrigger and afs.root for reactivity
  ...
  // Load decay warnings using afs.root from context
    ...
    <box flexGrow={1} flexDirection="column" overflow="hidden" onMouseDown={focusPane}>
      ...
      >
        <text fg={theme.textMuted}>
          {hivemindState()?.manifest?.stats?.totalEntries ?? 0} entries
        </text>
        <text fg={theme.textMuted}>h/l:tabs j/k:nav r:refresh 1-7:jump</text>
      </box>
    </box>
  )
}
...
function getStatusColor(status: string, theme: any): string {
  switch (status) {
    case "golden": return theme.success
    case "decaying": return theme.warning
    case "contested": return theme.error
...
function getCouncilStatusColor(status: string, theme: any): string {
  switch (status) {
    ...
    case "voting": return theme.info
    case "debating": return theme.warning
