---
title: Zen
description: Curated list of models provided by Oracle Code.
---
...
Oracle Code Zen is a list of tested and verified models provided by the Oracle Code team.

:::note
Oracle Code Zen is currently in beta.
:::

Zen works like any other provider in Oracle Code. You login to Oracle Code Zen and get
...
:::tip
We tested a select group of models and providers that work well with Oracle Code.
:::
...
Oracle Code Zen is an AI gateway that gives you access to these models.

---

## How it works

Oracle Code Zen works like any other provider in Oracle Code.

1. You sign in to **<a href={console}>Oracle Code Zen</a>**, add your billing
   details, and copy your API key.
2. You run the `/connect` command in the TUI, select Oracle Code Zen, and paste your API key.
...
The [model id](/docs/config/#models) in your Oracle Code config
uses the format `oracle-code/<model-id>`. For example, for GPT 5.1 Codex, you would
use `oracle-code/gpt-5.1-codex` in your config.
...
- Grok Code Fast 1 is currently free on Oracle Code for a limited time. The xAI team is using this time to collect feedback and improve Grok Code.
- Big Pickle is a stealth model that's free on Oracle Code for a limited time. The team is using this time to collect feedback and improve the model.
...
4. Have **no lock-in** by allowing you to use it with any other coding agent. And always let you use any other provider with Oracle Code as well.
