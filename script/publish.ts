...
if (!Script.preview) {
  ...
  const log =
    await $`git log v${previous}..HEAD --oneline --format="%h %s" -- packages/oracle-code packages/sdk packages/plugin`.text()
...
console.log("\n=== opencode ===\n")
await import(`../packages/oracle-code/script/publish.ts`)
...
if (!Script.preview) {
  ...
  await $`gh release create v${Script.version} --title "v${Script.version}" --notes ${notes.join("\n") ?? "No notable changes"} ./packages/oracle-code/dist/*.zip ./packages/oracle-code/dist/*.tar.gz`
}
