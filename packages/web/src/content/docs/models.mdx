...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Oracle Code.
...
Here are several models that work well with Oracle Code, in no particular order. (This is not an exhaustive list):
...
```jsonc title="oracle-code.jsonc" {7-12,19-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="oracle-code.jsonc" {6-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "oracle-code": {
...
When Oracle Code starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Oracle Code config.
