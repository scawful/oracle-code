...
There are two types of agents in Oracle Code; primary agents and subagents.
...
Oracle Code comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Oracle Code comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Oracle Code comes with two built-in primary agents and one built-in subagent.
...
Configure agents in your `oracle-code.json` config file:
...
- Global: `~/.config/oracle-code/agent/`
- Per-project: `.oracle-code/agent/`

```markdown title="~/.config/oracle-code/agent/review.md"
---
...
If no temperature is specified, Oracle Code uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Oracle Code config and the project specific config.
...
```json title="oracle-code.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="oracle-code.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="oracle-code.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `oracle-code models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/scawful/oracle-code).
:::
...
```markdown title="~/.config/oracle-code/agent/docs-writer.md"
---
...
```markdown title="~/.config/oracle-code/agent/security-auditor.md"
---
