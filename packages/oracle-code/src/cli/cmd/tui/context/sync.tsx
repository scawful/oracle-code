...
import type { Path } from "@oracle-code/sdk"
...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    // State API helpers (not in SDK types yet)
    async function refreshState() {
      try {
        const res = await fetch(`${sdk.url}/state`)
        if (res.ok) {
    ...
    const result = {
      ...
      state: {
        ...
        async set(key: string, value: string, section?: string) {
          await fetch(`${sdk.url}/state`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ key, value, section: section ?? "context" }),
          })
          await refreshState()
        },
        async remove(key: string) {
          await fetch(`${sdk.url}/state/${encodeURIComponent(key)}`, {
            method: "DELETE",
          })
          await refreshState()
        },
