...
import { createOracleCode, type ToolPart } from "@oracle-code/sdk"
...
console.log("🚀 Starting oracle-code server...")
const oracleCode = await createOracleCode({
  port: 0,
})
console.log("✅ OracleCode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await oracleCode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new oracle-code session...")
    const { client, server } = oracleCode
    ...
    console.log("✅ Created oracle-code session:", createResult.data.id)
  ...
  console.log("📝 Sending to oracle-code:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 OracleCode response:", JSON.stringify(result, null, 2))
