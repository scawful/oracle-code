    ...
    app
      ...
      .get(
        ...
        describeRoute({
          ...
          description: "Get all shared state entries for theory of mind coordination",
        ...
        async (c) => {
          ...
          if (!root) return c.json({ entries: [], lastUpdated: "" })
          const data = await State.getData(root)
          return c.json(data || { entries: [], lastUpdated: "" })
        },
      )
      .post(
        "/state",
        describeRoute({
          ...
          description: "Set a key-value entry in shared state",
          operationId: "state.set",
          responses: {
            200: {
              description: "Successfully set state entry",
              content: {
                "application/json": {
                  schema: resolver(z.boolean()),
                },
        ...
        validator(
          ...
          z.object({
            key: z.string(),
            value: z.string(),
            section: State.Section.optional().default("context"),
          }),
        ),
        async (c) => {
          const { key, value, section } = c.req.valid("json")
          const root = await AFS.findRoot()
          if (!root) return c.json(false)
          await State.set(root, key, value, section)
          return c.json(true)
        },
      )
      .delete(
        ...
        describeRoute({
          ...
          description: "Delete a specific entry from shared state",
          operationId: "state.delete",
          responses: {
            200: {
              description: "Successfully deleted state entry",
              content: {
                "application/json": {
                  schema: resolver(z.boolean()),
                },
        ...
        async (c) => {
          ...
          const root = await AFS.findRoot()
          if (!root) return c.json(false)
          const removed = await State.remove(root, key)
      ...
      .delete(
        "/state",
        describeRoute({
          summary: "Clear all state",
          description: "Clear all entries from shared state",
          operationId: "state.clear",
          responses: {
            200: {
              description: "Successfully cleared state",
              content: {
                "application/json": {
                  schema: resolver(z.boolean()),
                },
