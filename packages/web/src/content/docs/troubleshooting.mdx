...
To debug any issues with Oracle Code, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/oracle-code/log/`
- **Windows**: `%USERPROFILE%\.local\share\oracle-code\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `oracle-code --log-level DEBUG`.

---

### Storage

oracle-code stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/oracle-code/`
- **Windows**: `%USERPROFILE%\.local\share\oracle-code`
...
If you're experiencing issues with Oracle Code:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/scawful/oracle-code/issues**](https://github.com/scawful/oracle-code/issues)
...
### Oracle Code won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `oracle-code upgrade`
...
To figure out what models you have access to, run `oracle-code models`
...
If you encounter API call errors, this may be due to outdated provider packages. oracle-code dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart oracle-code to reinstall the latest provider packages

This will force oracle-code to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
oracle-code will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
