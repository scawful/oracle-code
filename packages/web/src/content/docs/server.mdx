---
title: Server
description: Interact with oracle-code server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `oracle-code serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an oracle-code client can use.
...
```bash
oracle-code serve [--port <number>] [--hostname <string>]
```
...
When you run `oracle-code` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the oracle-code server to interact with oracle-code programmatically.
:::

This architecture lets oracle-code support multiple clients and allows you to interact with oracle-code programmatically.

You can run `oracle-code serve` to start a standalone server. If you have the
oracle-code TUI running, `oracle-code serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Oracle Code [IDE](/docs/ide) plugins.
...
The oracle-code server exposes the following APIs.
