...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.OCODE_CONFIG_CONTENT) {
      result = mergeConfigWithPlugins(result, JSON.parse(Flag.OCODE_CONFIG_CONTENT))
      log.debug("loaded custom config from OCODE_CONFIG_CONTENT")
    }
    ...
    if (Flag.OCODE_CONFIG_DIR) {
      directories.push(Flag.OCODE_CONFIG_DIR)
      log.debug("loading config from OCODE_CONFIG_DIR", { path: Flag.OCODE_CONFIG_DIR })
    }
    ...
    if (Flag.OCODE_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OCODE_PERMISSION))
    }
  ...
  async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ["add", "@oracle-code/plugin@" + (Installation.isLocal() ? "latest" : Installation.VERSION), "--exact"],
      {
        cwd: dir,
      },
    ).catch(() => {})
  ...
  export const Keybinds = z
    .object({
      ...
      session_export: z.string().optional().default("<leader>se").describe("Export session to editor"),
      session_new: z.string().optional().default("<leader>sn").describe("Create a new session"),
      session_list: z.string().optional().default("<leader>sl").describe("List all sessions"),
      session_timeline: z.string().optional().default("<leader>st").describe("Show session timeline"),
      session_share: z.string().optional().default("none").describe("Share current session"),
      session_unshare: z.string().optional().default("none").describe("Unshare current session"),
      session_interrupt: z.string().optional().default("escape").describe("Interrupt current session"),
      session_compact: z.string().optional().default("<leader>sc").describe("Compact the session"),
      ...
      model_list: z.string().optional().default("<leader>ml").describe("List available models"),
      ...
      agent_list: z.string().optional().default("<leader>al").describe("List agents"),
      agent_cycle: z.string().optional().default("tab").describe("Next agent"),
      agent_cycle_reverse: z.string().optional().default("shift+tab").describe("Previous agent"),
      analysis_cycle: z.string().optional().default("<leader>xn").describe("Cycle analysis modes"),
      analysis_cycle_reverse: z.string().optional().default("<leader>xp").describe("Cycle analysis modes reverse"),
  ...
  export const global = lazy(async () => {
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "https://oracle-code.ai/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://oracle-code.ai/config.json"
